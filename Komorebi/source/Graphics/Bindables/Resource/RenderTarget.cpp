...
namespace gfx {
  ...
  RenderTarget::RenderTarget(IDXGISwapChain* swapChain) {
    ...
    m_width = (float)rtDesc.Width;
    m_height = (float)rtDesc.Height;

    m_viewport = memory::Factory::Create<Viewport>(0, 0, (int)m_width, (int)m_height);
    m_viewport->Setup();
  }
  ...
  void RenderTarget::Setup() {
    ...
    INFOMAN
      ...
      if (m_width < 0) {
        m_width = Engine::GetWindow()->GetClientWidth() * m_width * -1.f;
      }
    if (m_height < 0) {
      m_height = Engine::GetWindow()->GetClientHeight() * m_height * -1.f;
    }
    ...
    texDesc.Width = (int)m_width;
    texDesc.Height = (int)m_height;
    ...
    for (int i = 0; i < m_count; i++) {
      ...
      m_textures.push_back(memory::Factory::Create<Texture2D>(pTexture, m_format, m_width, m_height, m_slot + i, -1));
      m_tex2D.push_back(pTexture);
    }

    if (m_hasDS) {
      m_textures.push_back(memory::Factory::Create<Texture2D>(pDepthStencil, DXGI_FORMAT_R24_UNORM_X8_TYPELESS, m_width, m_height, m_slot + m_count));
    }
    ...
    if (m_mipLevels == 0u) {
      m_mipLevels = ((unsigned)log2f(max(m_width, m_height))) + 1u;
    }
  }
  ...
  void RenderTarget::Bind() const {
    ...
    if (m_rtv.size()) {
      GetContext()->OMSetRenderTargets(m_rtv.size(), m_rtv[0].GetAddressOf(), m_hasDS ? m_dsv.Get() : nullptr);
    }
    else {
  ...
  void RenderTarget::BindWithDS(const RenderTarget* rt) const {
    if (m_rtv.size()) {
      GetContext()->OMSetRenderTargets(m_rtv.size(), m_rtv[0].GetAddressOf(), rt->m_hasDS ? rt->m_dsv.Get() : nullptr);
    }
  ...
  void RenderTarget::SetMipLevel(unsigned int mipLevel) {
    INFOMAN
    if (m_currMipLevel == mipLevel) return;
    m_currMipLevel = mipLevel;
    for (int i = 0; i < m_tex2D.size(); i++) {
      D3D11_RENDER_TARGET_VIEW_DESC rtvDesc = {};
      if (m_hasDS && i == m_tex2D.size() - 1) {
        rtvDesc.Format = DXGI_FORMAT_R24G8_TYPELESS;
      }
      else {
        rtvDesc.Format = m_format;
      }
      ...
      rtvDesc.Texture2D.MipSlice = m_currMipLevel;
      GFX_THROW_INFO(GetDevice()->CreateRenderTargetView(m_tex2D[i].Get(), &rtvDesc, m_rtv[i].ReleaseAndGetAddressOf()));
    }

    m_viewport->SetSize((int)m_width >> m_currMipLevel, (int)m_height >> m_currMipLevel);
  }
  ...
  REFLECT_STRUCT_BEGIN(RenderTarget, StateBindable)
    ...
    REFLECT_STRUCT_MEMBER(m_hasDS)
    REFLECT_STRUCT_MEMBER(m_allowUAV)
