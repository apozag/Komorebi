...
ConstantBuffer::CBufferException::CBufferException(int line, const char* file) noexcept : Exception(line, file)
{}
const char* ConstantBuffer::CBufferException::GetType() const noexcept {
	return "Constant Buffer Update Exception";
}

ConstantBuffer::ConstantBuffer(std::vector<ConstantBufferVariable> variables, unsigned int slot) : m_dynamic(true), ResourceBindable(slot) {
...
bool ConstantBuffer::SetFloat(const char* name, float value) {
	return SetVariable(D3D_SVC_SCALAR, name, &value, sizeof(float));
}
bool ConstantBuffer::SetVector4(const char* name, float* data) {
	return SetVariable(D3D_SVC_VECTOR, name, data, 4 * sizeof(float));
}
bool ConstantBuffer::SetMat4(const char* name, float* data) {
...
bool ConstantBuffer::SetVariable(D3D_SHADER_VARIABLE_CLASS varClass, const char* name, void* data, size_t size) {
	try {
