...
DirectionalLight::DirectionalLight(Graphics& gfx, DirectX::SimpleMath::Vector3 color) :
	color(color)
{
	m_rt = new RenderTarget(gfx, 512, 512, DXGI_FORMAT_R32G32B32A32_FLOAT, 0);
	m_camera = new Camera(gfx, 1.0472f, 1, 0.1, 50, m_rt, true);
}
...
SpotLight::SpotLight(Graphics& gfx, DirectX::SimpleMath::Vector3 color, float phi, float theta) :
	...
	phi(phi),
	theta(theta),
	m_rt(RenderTarget(gfx, 512, 512, DXGI_FORMAT_A8_UNORM, 0)),
...
PointLight::PointLight(Graphics& gfx, DirectX::SimpleMath::Vector3 color) : 
	color(color),
	m_rts{
			RenderTarget(gfx, 512, 512, DXGI_FORMAT_A8_UNORM, 0),
			RenderTarget(gfx, 512, 512, DXGI_FORMAT_A8_UNORM, 0),
			RenderTarget(gfx, 512, 512, DXGI_FORMAT_A8_UNORM, 0),
			RenderTarget(gfx, 512, 512, DXGI_FORMAT_A8_UNORM, 0),
			RenderTarget(gfx, 512, 512, DXGI_FORMAT_A8_UNORM, 0),
