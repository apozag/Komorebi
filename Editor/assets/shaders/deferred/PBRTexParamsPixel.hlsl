...
Texture2D metalnessTex : register(t9);
...
PS_OUTPUT main(VSout i) {
  ...
  float roughnessVal = roughnessTex.Sample(texSampler, i.uv) * roughness;
  float metalnessVal = metalnessTex.Sample(texSampler, i.uv) * metalness;
  o.pbrParams = float4(metalnessVal, roughnessVal, reflectivity, 1);
  return o;
}